let common = {
    ...
    user_edit_update: (user_id = 0) => {
        ...
        requiredFields.forEach(id => {
            ...
            if (!input) return;
            if (input.value.trim() === '') {
                input.classList.add('error-field'); // подсветка
                valid = false;
            } else {
        ...
        if (!valid) {
            ...
            const firstError = qs('.error-field');
        ...
        let data = {
            user_id: user_id,
            first_name: gv('first_name').trim(),
            last_name: gv('last_name').trim(),
            phone: gv('phone').trim(),
            email: gv('email').trim(),
            plot_id: gv('plot_id').trim(),
            offset: global.offset
        };
    ...
    user_delete_action: (user_id) => {
        // vars
        let data = {
            user_id: user_id,
            offset: global.offset
        };
        let location = {dpt: 'user', act: 'delete_action'};
        // call
        request({location: location, data: data}, (result) => {
            common.modal_hide();
            html('table', result.html);
