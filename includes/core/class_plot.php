...
class Plot {
    ...
    public static function plots_convert_numbers_to_ids($raw) {
        ...
        $pdo = DB::connect();
        $quoted = array_map([$pdo, 'quote'], $items);
        $in = implode(',', $quoted);

        $q = DB::query("SELECT plot_id FROM plots WHERE number IN ($in) ORDER BY plot_id;") or die(DB::error());
        $ids = [];
    ...
    public static function plots_convert_ids_to_numbers($raw) {
        $items = array_map('trim', explode(',', $raw));
        $items = array_filter($items, fn($v) => $v !== '');
        if (empty($items)) return '';
        ...
        $q = DB::query("SELECT number FROM plots WHERE plot_id IN ($in) ORDER BY plot_id;") or die(DB::error());
